...
class Solver(object):
    ...
    def build_model(self):
        ...
        if self.dataset in ['CelebA', 'RaFD']:
            self.Gs = [Generator(self.g_conv_dim, self.c_dim, self.g_repeat_num) for i in range(5)]
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            d_loss_fake = 0
            for g in self.Gs:
                x_fake = g(x_real, c_trg, c_org)
                out_src = self.D(x_fake.detach(), c_trg)
                d_loss_fake += F.relu(1 + out_src.mean()) / len(self.Gs)
            ...
            if (i+1) % self.n_critic == 0:
                for g, g_opt in zip(self.Gs, self.g_opts):
                    ...
                    x_reconst = g(x_real, c_org, c_org)
                ...
                with torch.no_grad():
                    ...
                    for c_fixed in c_fixed_list:
                        ...
                        for g in self.Gs:
                            ...
                            score = self.D(fake, c_fixed) - 10 * (recon-x_fixed).abs().mean()
                            if score >= best_score:
                                best = fake
                                best_score = score

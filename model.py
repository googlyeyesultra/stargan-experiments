...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_size=128, conv_dim=64, c_dim=5, repeat_num=6):
        ...
        curr_dim = conv_dim
        for i in range(repeat_num - 1 - repeat_num // 3):
    ...
    def forward(self, x):
        x = self.layers(x)
        out_src = self.tails[-1](x).squeeze((1, 2, 3))
        out_cls = torch.stack([self.tails[i](x).squeeze((1, 2, 3)) for i in range(self.c_dim)], dim=1)
